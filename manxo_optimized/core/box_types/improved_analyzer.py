...
# -*- coding: utf-8 -*-
"""
Max/MSP ボックスタイプ分析ツール (改良版)
compass_artifact_wf-1fdfe7bf-577c-4c32-a8eb-3cb326ff631f_text_markdown.md に基づく実装

このスクリプトは、Max/MSPのパッチファイルを分析し、各ボックスのタイプを正確に識別します。
...
class ImprovedBoxTypeAnalyzer:
    """Max/MSPボックスタイプの高精度分析クラス"""
    ...
    def _analyze_patch_data(self, patch_data: Dict[str, Any], patch_name: str) -> Dict[str, Any]:
        ...
        # 各ボックスについて分析
        # boxesはdict形式またはlist形式の可能性がある
        if isinstance(boxes, dict):
            box_items = boxes.items()
        elif isinstance(boxes, list):
            box_items = enumerate(boxes)
        else:
            box_items = []
            
        for box_id, box_data in box_items:
            # box形式の確認（patcherバージョンによって異なる）
            box_content = box_data.get("box", box_data) if isinstance(box_data, dict) else box_data
            ...
            if "patcher" in box_content:
                ...
                subpatch_name = f"{patch_name}_{box_id}"
    ...
    def generate_report(self, results: List[Dict[str, Any]], output_dir: str = None) -> str:
        ...
        if 'maxclass' in df.columns:
            maxclass_counts = df['maxclass'].value_counts()
            report += "| maxclass | カウント | パーセント |\n"
            report += "|----------|----------|------------|\n"
            for mc, count in maxclass_counts.items():
        ...
        if 'object_type' in df.columns:
            object_types = df['object_type'].dropna().value_counts()
            if not object_types.empty:
                report += "| オブジェクトタイプ | カウント | パーセント |\n"
                report += "|------------------|----------|------------|\n"
                newobj_total = object_types.sum()
                for obj, count in object_types.head(30).items():  # 上位30種類のみ表示
        ...
        if 'object_category' in df.columns:
            category_counts = df['object_category'].dropna().value_counts()
            if not category_counts.empty:
                report += "| カテゴリ | カウント | パーセント |\n"
                report += "|----------|----------|------------|\n"
                for cat, count in category_counts.items():
                    percent = (count / len(df['object_category'].dropna())) * 100
...
def main():
    """メイン実行関数"""
    parser = argparse.ArgumentParser(description="Max/MSP パッチファイルのボックスタイプ分析ツール")
    parser.add_argument("input", help="分析する.maxpatファイルまたはディレクトリのパス")
    parser.add_argument("--output", "-o", help="レポート出力ディレクトリ", default="./output")
    parser.add_argument("--pattern", "-p", help="ディレクトリ指定時のファイルパターン", default="*.maxpat")
    ...
    if os.path.isdir(args.input):
        # ディレクトリ内の全ての.maxpatファイルを分析
        import glob
        maxpat_files = glob.glob(os.path.join(args.input, args.pattern), recursive=True)
        print(f"{len(maxpat_files)}個のパッチファイルを分析します...")
        
        for file_path in maxpat_files:
            try:
                result = analyzer.analyze_patch_file(file_path)
                if result:
    ...
    if results:
        ...
        print("分析結果がありません。有効なMaxパッチファイルを指定してください。")
